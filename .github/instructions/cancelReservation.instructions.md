---
applyTo: src/UseCase/CancelReservation/**/*.php
---
...
[POST] https://sms-api.aossms.com/p5/api/cancelreservation.json
...
| clientTag | string(1 - 200) | 送信ステータス確認用の任意の識別文字列を指定します。※本項目または scheduleTime, scheduleDate のいずれかが必須。※優先度は clientTag > scheduleTime > scheduleDate になります。※本項目を指定した場合、指定文字列を保持する予約送信をキャンセルします。 |
...
| groupTag | string(1 - 200) | 予約リクエスト時に指定した任意識別文字列を指定します。一括キャンセルする場合に使用します。 |
...
| 130 | Already send. | キャンセル対象のクライアントタグは既に送信されています。 |
